...
<CardGroup cols={2}>
  <Card title="Arcade Tool" icon="gamepad-modern" href="/tools/automation/arcadetool">
    Access 100+ third-party tools and services through Arcade's unified API platform.
  </Card>
...
arcade_manager = ArcadeToolManager(default_user_id="user@example.com")
arcade_tools = arcade_manager.get_tools(toolkits=["Slack", "Google"])
...
agent = Agent(
    ...
    tools=[*arcade_tools, apify_automation, platform_integration, browser_automation],
